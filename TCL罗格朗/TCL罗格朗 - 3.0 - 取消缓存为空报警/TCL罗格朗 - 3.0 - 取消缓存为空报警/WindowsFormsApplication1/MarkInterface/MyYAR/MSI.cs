...
namespace HG.MSICS
{
    ...
    public class MSI
    {
        ...
        public static bool Loadmodelfile(ref string path)
        {
            m_mksIndex = NativeMethods.ELM_OpenMksFile(path);
            if (m_mksIndex >= 0)
            {
                return true;
            }
        ...
        public static bool SetPresaveMode()
        {
            if (m_mksIndex >= 0)
            {
                ...
                NativeMethods.ELM_PrepareMark(0, m_mksIndex);
                return true;
            }
            else
            {
       ...
       public static bool MarkFlyByInIOSignal(bool withtrriger)
       {
           if (withtrriger)
           {
               long piSource = 0;
               ...
               if (trrigerresult == 1)
               {
                   if (isfirstmark)
                   {
                       NativeMethods.ELM_MarkMksFile(0, m_mksIndex, 0);
                       isfirstmark = false;
                   }
                   else
                   {
                       NativeMethods.ELM_MarkMksFile(0, m_mksIndex, 2);
                   }
                   return true;
               }
               else
           ...
           else
           {
               ...
               if (ir > 0)
               {
                   NativeMethods.ELM_WaitForMarkEnd(0, 100000);
                   return true;
               }
               else
               {
        ...
        public static bool ChangeTextByName(ref string Name,ref string txt)
        {
        ...
        public static bool DrawMksFile(IntPtr HWnd)
        {
