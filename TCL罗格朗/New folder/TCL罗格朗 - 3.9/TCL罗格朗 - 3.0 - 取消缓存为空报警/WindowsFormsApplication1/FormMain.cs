...
namespace WindowsFormsApplication1
{
    public partial class FormMain : Form
    {
        ...
        private SerialPortInfo cominfoMES = new SerialPortInfo();
        private CommInterface ComworkMES = new CommSerial();

        private SerialPortInfo cominfoIO = new SerialPortInfo();
        ...
        private void INITWINDOWSFORM()
        {
            ...
            comboBoxIOCARDCOM.Text = cf.ReadConfig("COMSET", "COMPORTIOCARD", comboBoxIOCARDCOM.Text);
            comboBoxIOCARDBAR.Text = cf.ReadConfig("COMSET", "COMBATEIOCARD", comboBoxIOCARDBAR.Text);
        ...
        private void CommunicationIint()
        {
            ...
            cominfoMES.PortName = comboBox1.Text;
            cominfoMES.BaudRate = Convert.ToInt32(comboBox2.Text);

            (ComworkMES as CommSerial).PortInfo = cominfoMES;
            (ComworkMES as CommSerial).SetDataReceived(SerialDataReceivedfromiobord);
            if (ComworkMES.Open())
            {
                log.WriteLog("Com mes successful",textBoxlog);
           
            }
            else
            {
               MessageBox.Show("Com mes fail");
               this.Close();
            }

            cominfoIO.PortName = comboBoxIOCARDCOM.Text;
            cominfoIO.BaudRate = Convert.ToInt32(comboBoxIOCARDBAR.Text);
            (ComworkIO as CommSerial).PortInfo = cominfoIO;
            (ComworkIO as CommSerial).SetDataReceived(SerialDataReceivedfromsaoma);
                                ...
                                if (getbytefromlist[q] == 0x1b && getbytefromlist[q + 1] == 0x03 && getbytefromlist[q + 2] == 0x0a)//find end  code 
                                {
                                    ...
                                    lock (lockprintstring)
                                    {
                                        if (printstring.Count >= Convert.ToInt32(textBoxdatanumberallow.Text))
                                        {
                                            ...
                                            b.DoWork += new DoWorkEventHandler(DoWorkIO);
                                            b.RunWorkerAsync();
                                        }
                                        else
                                        {
        ...
        private void buttonIOCARDSET_Click(object sender, EventArgs e)
        {
            Configure cf = new Configure();
            cf.WriteConfig("COMSET", "COMPORTIOCARD", comboBoxIOCARDCOM.Text);
            cf.WriteConfig("COMSET", "COMBATEIOCARD", comboBoxIOCARDBAR.Text);
            MessageBox.Show("OK");
        }
