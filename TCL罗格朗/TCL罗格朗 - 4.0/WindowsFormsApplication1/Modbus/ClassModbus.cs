...
namespace WindowsFormsApplication1.Modbus
{
    class modbus
    {
        ...
        private ModbusCommSerial mobuscom = null;

        public void OnGettingArrybyteByModbus(out byte[] arry)
        {
            ReadFromserialport();
            byte[] mess = _arry.ToArray();
            arry = mess;

        }


        public void RepalyGettingArrybyteByModbus(ref byte[] arry)
        {
            Thread.Sleep(11);
            mobuscom.Write(arry);
            Thread.Sleep(11);
        ...
        private string comstringname = null;
        int baudrate = 0;
        public void Setportparameters(string mcomname, int mbaudrate)
        {
            comstringname = mcomname;
            baudrate = mbaudrate;
        }
        ...
        {
            isbreak = false;
            ModbusSerialPortInfo info = new ModbusSerialPortInfo();
            info.PortName = comstringname;
            info.BaudRate = baudrate;

            mobuscom = new ModbusCommSerial();
            (mobuscom as ModbusCommSerial).PortInfo = info;
            ...
            {
                MessageBox.Show("MODBUS协议串口打开失败");
                return;
            }

        }

        private List<byte> _arry = new List<byte>();
        ...
        private void ReadFromserialport()
        {
            ...
            {
                byte[] arry;              
                mobuscom.Read(out arry);
                _arry.AddRange(arry);

                //if (_arry.Count >= 6)
                //{
                //    byte[] mess = _arry.ToArray();
                //    if (CheckResponse(mess))
                //    {
                //        break;
                //    }
                //    else
